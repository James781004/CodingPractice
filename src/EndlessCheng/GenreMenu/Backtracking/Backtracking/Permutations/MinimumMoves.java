    ...
    public int minimumMoves(int[][] grid) {
        ...
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[i].length; j++) {
                if (grid[i][j] > 1) { // 移走的石子的坐標（可能有重復的坐標）
                    for (int k = 1; k < grid[i][j]; k++) {
                        from.add(new int[]{i, j});
                    }
                } else if (grid[i][j] == 0) { // 移入的石子的坐標
                    to.add(new int[]{i, j});
                }
            }
        }
        ...
        for (List<int[]> from2 : permutations(from)) { // 枚舉 from 的所有排列，與 to 匹配
            int total = 0;
            for (int i = 0; i < from2.size(); i++) { // 累加從 from[i] 到 to[i] 的曼哈頓距離
                int[] f = from2.get(i);
                int[] t = to.get(i);
